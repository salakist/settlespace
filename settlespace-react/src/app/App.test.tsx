...
test('renders authentication shell when user is unauthenticated', () => {
  ...
  expect(screen.queryByText(/SettleSpace Person Manager/i)).not.toBeInTheDocument();
});
...
test('does not pass logged-in user to persons page list', () => {
  ...
  const personsPageProps = mockPersonsPage.mock.calls[0]?.[0] as
    | {
        persons?: Array<{ id: string; firstName: string; lastName: string; addresses: unknown[] }>;
      }
    | undefined;
