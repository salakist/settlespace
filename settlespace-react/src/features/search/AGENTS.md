...
## Responsibilities
- `src/features/search/components/SearchBar.tsx` — reusable search input and high-level composition for the shared search UI.
...
- `src/features/search/hooks/useUrlSearchQuery.ts` — generic hook that wraps `useSearchParams` with a parse/serialize pair to sync a typed query object ↔ URL search params. Used by route containers (`PersonsRoutePage`, `TransactionsRoutePage`) to drive search from the URL.
...
- Prefer the declarative bridge field shapes (`text-single`, `text-multi`, `lookup-single`, `lookup-multi`, `resolved-single`, `resolved-multi`) plus the small `custom` escape hatch before adding more one-off bridge helpers.
...
- Supported `kind` values:
  ...
  - `date-input` — use the shared date picker / exact-date entry flow before applying the chip
