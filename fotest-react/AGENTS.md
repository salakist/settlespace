...
- Authenticated password change UI in `ChangePasswordForm.tsx`
- Person form handling in `PersonForm.tsx`
- Listing and action controls in `PersonList.tsx`
- Search flow in `SearchBar.tsx`
- API calls, login, password change, token persistence, and auth header attachment in `api.ts`
...
Authenticated users can change their own password from the header actions.
